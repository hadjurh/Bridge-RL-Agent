...
if __name__ == '__main__':
    ...
    for file in files:
        try:
            ...
            print("Error: File parameter should only specify .game files")
            exit(1)
        ...
        # If you want to remove .game and .score files:
        # os.remove(str(file))
        # os.remove("database/" + file_name_no_extension + ".score")

        if not os.path.exists('database-qlearning'):
            os.makedirs('database-qlearning')

        with open('database-qlearning/learn_' + str(number_of_games) + "_" +
