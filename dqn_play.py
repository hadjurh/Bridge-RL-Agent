    ...
    for _ in range(nb_of_games):
        ...
        while True:
            ...
            if heuristic_strategy:
                ...
                best_indices = np.argsort(-memory_agent.model.predict(observation))

                dominant_cards = [card for card in current_player.hand if card.suit in game.dominant_suit]

                # If player has dominant cards
                # Play the best one
                if dominant_cards:
                    observations = [card.observation() - 1 for card in dominant_cards]
                    current_card = current_player.play_card(observation_to_card(
                        next(index for index in best_indices[0] if index in observations) + 1))

                # If player has no dominant cards
                # Play the best one
                else:
                    observations = [card.observation() - 1 for card in current_player.hand]
                    current_card = current_player.play_card(observation_to_card(
                        next(index for index in best_indices[0] if index in observations) + 1))
